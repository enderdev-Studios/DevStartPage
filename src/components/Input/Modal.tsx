export default function Modal({ state, onSubmit, onClick }: { state: boolean, onSubmit: any, onClick: any }) {
    ...
    return (
        <div className={backdrop}>
                <div className={`modal background-crust subtext-0 ${modalState} max-md:mx-4`} >
                    ...
                    <h3 className="text-2xl font-bold ml-2 max-md:text-lg">Crear Acceso Directo</h3>
                    <form
                        className="flex flex-col m-3"
                        onSubmit={onSubmit}>
                        <label className="text-lg">Nombre:</label>
                        <input type="text" name="name" placeholder="Google" className="mb-2 p-2 rounded m-2 outline-0 focus:border-b-2 focus:border-b-overlay-latte-0 text" required autoComplete="off" />
